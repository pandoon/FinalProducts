...
file_path = r"PATH" #Replace PATH with your file path
...
def process_time_series(symbol, interval='1month', outputsize=172, timezone='America/New_York'):
    ...
    time.sleep(5) #Just so you dont use all the api credits per minute.
    return df
...
def call_stock(file_path):
    ...
    for sheet_name, ticker in zip(sheet_names[:], tickers[:]):
        ...
        if 'Period End' in stock_sheet.columns and 'Period End' in data_sheet.columns:
            ...
            cols += ['close', 'volume']
            merged = merged[cols]
            if 'CommonStockSharesOutstanding' in merged.columns and 'close' in merged.columns:
                # Calculate Market Cap
                merged['Market Cap'] = merged['CommonStockSharesOutstanding'] * merged['close']
            
            output_file_path = f"C:\\Users\\thoma\\Desktop\\Projects\\Project Oversight\\ratios.xlsx" #REPLACE WITH YOUR PATH
